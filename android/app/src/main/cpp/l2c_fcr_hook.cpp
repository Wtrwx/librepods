...
tBTA_STATUS fake_BTA_DmSetLocalDiRecord(tSDP_DI_RECORD *p_device_info, uint32_t *p_handle) {
    ...
    if (!original_BTA_DmSetLocalDiRecord) {
        LOGE("fake_BTA_DmSetLocalDiRecord: original function is unavailable");
        return BTA_FAILURE;
    }

    if (enableSdpHook.load(std::memory_order_relaxed) && p_device_info) {
    ...
    // The original implementation must be invoked exactly once. Calling it repeatedly creates
    // duplicate SDP records and can destabilize the Bluetooth process.
    const tBTA_STATUS status = original_BTA_DmSetLocalDiRecord(p_device_info, p_handle);
