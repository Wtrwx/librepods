...
class AACPManager {
    ...
    companion object {
        ...
        object Opcodes {
            ...
            const val TIPI_SCORE: Byte = 0x02
        }
        ...
        enum class TipiScore(val value: Byte) {
            LOW(0x02), HIGH(0x06), MAX(0x07), SMART_ROUTING_DISABLED(0x08)
        }
    ...
    /** Sends the SR 2.0 score directly to the AirPods firmware. */
    fun sendTipiScoreToAirPods(score: TipiScore = TipiScore.LOW): Boolean {
        Log.d(TAG, "Sending direct TiPi score to AirPods: ${score.name} (${score.value})")
        return sendDataPacket(
            byteArrayOf(
                Opcodes.TIPI_SCORE,
                0x00,
