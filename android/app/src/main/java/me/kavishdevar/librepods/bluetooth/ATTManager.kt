...
class ATTManagerv2 {
    ...
    @Volatile private var readerSocket: BluetoothSocket? = null
    ...
    fun startReader(socket: BluetoothSocket) {
        if (readerRunning.get() && readerSocket === socket) return
        stopReader()
        readerSocket = socket
    ...
    fun stopReader(expectedSocket: BluetoothSocket? = null) {
        if (expectedSocket != null && readerSocket !== expectedSocket) return
        readerRunning.set(false)
    ...
    fun disconnected(socket: BluetoothSocket? = BluetoothConnectionManager.attSocket) {
        ...
        if (BluetoothConnectionManager.attSocket === socket) {
            BluetoothConnectionManager.attSocket = null
        }
        Log.d(TAG, "ATT disconnected")
    }

    private fun runReaderLoop(socket: BluetoothSocket) {
        val input = socket.inputStream
        val buffer = ByteArray(512)

        while (readerRunning.get() && readerSocket === socket) {
            try {
