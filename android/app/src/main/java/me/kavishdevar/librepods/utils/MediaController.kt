        ...
        override fun onPlaybackConfigChanged(configs: MutableList<AudioPlaybackConfiguration>?) {
            ...
            if (pausedForOtherDevice) {
                ...
                if (isActive) {
                    ...
                    if (hasNewMusicOrMovie) {
                        // Genuine local music/movie playback is an explicit user take-over
                        // intent, so act immediately instead of deferring by OWNERSHIP_RETRY_MS.
                        // The recentlyLostOwnership guard only exists to swallow spurious
                        // replays that carry no new media, handled by the else branch below.
                        ...
                        Log.d("MediaController", "Recently lost ownership without new media; scheduling deferred take-over")
                        handler.removeCallbacks(deferredTakeOverRunnable)
                        handler.postDelayed(deferredTakeOverRunnable, OWNERSHIP_RETRY_MS)
                    } else {
                        Log.d("MediaController", "Skipping take-over; no new music/movie")
                    }
                } else {
